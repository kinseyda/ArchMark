...
class ArrowMarker(val arrow: Arrow, var targetView: TargetView) {
    private val cmCoords = ConversionUtils.cmPolarToCoordinates(arrow.angle, arrow.distance, targetView.targetFace)
    private val markerCoords = ConversionUtils.cmToPixel(cmCoords.first, cmCoords.second, targetView, targetView.targetFace)
